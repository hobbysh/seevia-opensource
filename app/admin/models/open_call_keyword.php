 ...
 * $Id$
*****************************************************************************/
class OpenCallKeyword extends AppModel
{
    /*
     * @var $useDbConfig 数据库配置
     */
    public $useDbConfig = 'sns';
    public $name = 'OpenCallKeyword';
/*
    var $hasOne = array('PageModuleI18n' =>
                        array('className'    => 'PageModuleI18n',
                              'conditions'    =>  '',
                              'order'        => 'PageModule.id',
                              'dependent'    =>  true,
    ...
    function tree($locale,$conditions){
    ...
    function parent_tree($locale,$code){
        $this->modules_parent_format=array();
        $this->set_locale($locale);
        if(!empty($code)){
            $modules=$this->find("all",array('conditions'=>array('PageModule.type'=>'module_parent','PageModule.page_style_code'=>$code),'order' => 'PageModule.orderby asc,PageModule.created asc'));
        }else{
     ...
     */
     /*
    function subcat_get($category_type_id){
        $subcat=array();
    ...
    */
    /*
    function get_flash_module($locale){
        $this->set_locale($locale);
