...
class wechatauth
{
	...
	private $_datapath = 'cookiecode_';
	private $debug;
	private $_logcallback;
	public $login_user; //当前登陆用户, 调用get_login_info后获取
	private $web_host;//服务器路径
	...
	public function __construct($options)
	{
		...
		$this->web_host = isset($options['web_host'])?$options['web_host']:'';
	...
	*/
	public function saveCookie($filename,$content){
		return setcookie($filename,$content,time()+3600);
	}
	/**
	* 读取cookie缓存内容
	* @param string $filename 缓存文件名
	...
	*/
	public function get_login_code(){
		if (isset($this->_logincode)&&!empty($this->_logincode)) return $this->_logincode;
	...
	*/
	public function get_login_info(){
		...
		$post = array(
			"BaseRequest"=>array(
				array(
					"Uin"=>isset($citems['wxuin'])?$citems['wxuin']:0,
					"Sid"=>isset($citems['wxsid'])?$citems['wxsid']:0,
					"Skey"=>$this->skey,
					"DeviceID"=>''
				)
			)
		...
		$result['User']['Sid']=isset($citems['wxsid'])?$citems['wxsid']:0;
	...
	*/
	public function get_avatar($url,$uin) {
		if (!$this->cookie) return false;
		...
		$result = $send_snoopy->results;
		if ($result){
			$img_avatar=md5(date("Y-m-d H:i:s").'wechat'.$uin).".jpg";
			$img_root="img/users/".date('Ym')."/";
			$imgaddr = $this->web_host."img/users/".date('Ym')."/";
	    	$this->mkdirs($imgaddr);
			@chmod($imgaddr, 0777);
			$img_filename=$imgaddr.$img_avatar;
			$fp=fopen($img_filename, "w+"); //打开文件指针，创建文件
			if (!is_writable($img_filename) ){
			      return false;
			}
			fclose($fp);//关闭指针
			if (!file_exists($img_filename)) {
				...
				if (is_writable($img_filename)) {
				    file_put_contents ($img_filename, $result);
				    return '/'.$img_root.$img_avatar;
				}else{
					return false;
				}
			}
	...
	*/
	...
	/*
		整理用户信息
	*/
	function wechatauth_callback(){
		if(!empty($this->_login_user)&&$this->_login_user['Uin']!=0){
			$userInfo=$this->_login_user;
			$userInfo['HeadImg']="https://wx.qq.com".$userInfo['HeadImgUrl'];
			$userInfo['Img']=$this->get_avatar($userInfo['HeadImgUrl'],$userInfo['Uin']);
			$data['auth']=array(
					'provider' => 'wechatauth',
					'uid' => $userInfo['Uin'],
					'info' => array(
