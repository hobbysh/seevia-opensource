...
class alipay {
    ...
    function go($order, $payment_config){
		...
		if($payment_config['foo']==1){
			$parameter = array(
		        ...
		        "notify_url"		=> isset($payment_config['notify_url'])?$payment_config['notify_url']:$payment_config['return_url'],
		        "_input_charset"	=> $this->_input_charset,
		        "show_url"			=> $show_url,
		        //从订单数据中动态获取到的必填参数
		        "out_trade_no"		=> $host.'-'.date('Y-m-d').'-'.$order['id'],
		...
		$this->is_wechat=isset($payment_config['is_wechat'])?$payment_config['is_wechat']:'0';
		//构造请求函数
	...
	*return 表单提交HTML文本
     */
    function build_form2() {
		//GET方式传递
        $sHtml = "<form id='payform' name='payform' action='".$this->gateway."_input_charset=".$this->parameter['_input_charset']."' method='get'>";
		//POST方式传递（GET与POST二必选一）
		//$sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->gateway."_input_charset=".$this->parameter['_input_charset']."' method='post'>";
        ...
        $sHtml = $sHtml."<input type='hidden' name='sign' value='".$this->mysign."'/>";
        $sHtml = $sHtml."<input type='hidden' name='sign_type' value='".$this->sign_type."'/>";
		$sHtml = $sHtml.'<input type="submit" style="" value="支付" />';
	...
	*return 服务器ATN结果集
	...
	function get_track_id(){
		$host = isset($_SERVER['HTTP_X_FORWARDED_HOST']) ? $_SERVER['HTTP_X_FORWARDED_HOST'] : (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : '');
		$strlength=strlen($host.'-'.date('Y-m-d'))+1;
		return substr($this->response['out_trade_no'],$strlength);
	}
