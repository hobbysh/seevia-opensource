...
class paypalcn{
	...
	function build_form($order){
		...
		$currency_code='CNY';
		$sHtmlText.='<form name="payform" id="payform" style="text-align:center;" action="https://www.paypal.com/cn/cgi-bin/webscr" method="post" >';
		...
		$sHtmlText.= "<input type='submit'  value='确认付款'></form>";	
	...
	function build_form2($order){
		$sHtmlText="";
		$currency_code='CNY';
		$sHtmlText.='<form name="payform" target="_blank" id="payform" style="text-align:center;" action="https://www.paypal.com/cn/cgi-bin/webscr" method="post" >';
		$sHtmlText.="<input type='hidden' name='cmd' value='_xclick'/>";
		$sHtmlText.="<input type='hidden' name='business' value='".$this->busy."'/>";
		$sHtmlText.="<input type='hidden' name='item_name' value='".$order['subject']."'/>
		<input type='hidden' name='amount' value='".$order['amount']."'/>
		...
		<input type='hidden' name='rm' value='2'/>
		<input type='hidden' name='cancel_return' value='".$this->cancel_return."' /><!-- 取消页面 -->";
		$sHtmlText.='<input type="submit" style="font-size:15px;width:140;height:50" value="付款" />';		
		$sHtmlText.= "</form>";	
