...
window.onload = function () {
    ...
    var ui_1 = getid('user_interface_1');
    var ui_2 = getid('user_interface_2');
    ...
    if (ui_1)
    {
        ui_1.onclick = function ()
        {
            ...
            {
                getid('ucenter').style.display = 'none';
            }
        }
    }
    if (ui_2)
...
{
   var f = getid("js-setup");
   if (getid('user_interface_1').checked === true)
   {
   ...
   {
       getid("ucapinotice").innerHTML='请填写UCenter的URL';
        uccheck = false;
   }
   else
   {
        getid("ucapinotice").innerHTML='';
   }

    if(f["js-ucip"] && f["js-ucip"].value.length < 1) {
        getid("ucipnotice").innerHTML='请填写UCenter的IP';
        uccheck = false;
    }
    
    if (f['js-ucfounderpw'].value.length < 1)
    {
        getid("ucfounderpwnotice").innerHTML='请填写 UCenter 创始人的密码';
        uccheck = false;
    }
    else
    {
        getid("ucfounderpwnotice").innerHTML='';
    }
...
function displayres(res)
{
    if (res.error !== 0)
    {
        getid("ucfounderpwnotice").innerHTML= res.message;
        if(res.error == 2) {
            ...
            getid("ucip").appendChild(td1);
            getid("ucip").appendChild(td2);
        }
        
    }
    else
    {
    	var ui = (getid('user_interface_1').checked === true)?getid('user_interface_1').value:getid('user_interface_2').value;
